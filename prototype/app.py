...
def load_global_scaler():
    """Loads the main preprocessing scaler generated by the user pipeline."""
    path = BASE_DIR / "scaler.pkl"
...
def check_abnormal_prediction(pred):
    if pred < 0 or pred > 500000:
        st.warning("⚠️ **Abnormal Prediction Detected!** The model output is wildly out of bounds.")

def get_smart_volume(raw_volume, model):
    """
    Checks if the model handles scaling internally (Pipeline). 
    If not, it tricks the global scaler into scaling the volume manually.
    """
    if isinstance(model, Pipeline):
        return raw_volume  # The pipeline will scale it internally
    
    if global_scaler is not None:
        # Global scaler expects ['Open', 'High', 'Low', 'Volume']
        dummy_input = np.zeros((1, 4))
        dummy_input[0, 3] = raw_volume 
        return global_scaler.transform(dummy_input)[0, 3]
    return raw_volume
...
def run_recursive_forecast(algo, model, seed_prices, seed_volumes, anchor_date, n_days):
    ...
    for _ in range(n_days):
        ...
        raw_vol_forecast = float(np.mean(vol_hist))
        
        # Get scaled or raw volume depending on model architecture
        vol_to_use = get_smart_volume(raw_vol_forecast, model)
        ...
        if algo == ALGO_LR:
            ...
            X = pd.DataFrame([[vol_to_use, month, day, vol7, return_lag1]], columns=FEATURES[ALGO_RF])
            ...
            vol_mom = raw_vol_forecast / np.mean(vol_hist)  # KNN uses momentum ratio, scaling cancels out
            ...
            mom7 = float((price_hist[-1] / price_hist[-8]) - 1) if len(price_hist) >= 8 else 0.0
            X = pd.DataFrame([[vol_to_use, vol7, ret_lag1, mom7]], columns=FEATURES[ALGO_GB])
        ...
        vol_hist = (vol_hist + [raw_vol_forecast])[-10:]
...
with tab_compare:
    ...
    if not rows:
        st.error("No metrics files found under prototype/summary_metrics/.")
    else:
...
with tab_predict:
        ...
        if lr_do_predict or lr_do_forecast:
            ...
            if err:
                ...
                if lr_do_predict:
                    ...
                    vol_to_use = get_smart_volume(lr_volume, models[ALGO_LR])
                    X = pd.DataFrame([[vol_to_use, final_month, final_day, vol7, ma7]], columns=FEATURES[ALGO_LR])
                    ...
                    check_abnormal_prediction(pred)
        ...
        if rf_do_predict or rf_do_forecast:
            ...
            if err:
                ...
                if rf_do_predict:
                    ...
                    vol_to_use = get_smart_volume(rf_volume, models[ALGO_RF])
                    X = pd.DataFrame([[vol_to_use, final_month, final_day, vol7, return_lag1]], columns=FEATURES[ALGO_RF])
        ...
        if gb_do_predict or gb_do_forecast:
            ...
            if err:
                ...
                if gb_do_predict:
                    ...
                    vol_to_use = get_smart_volume(gb_volume, models[ALGO_GB])
                    X = pd.DataFrame([[vol_to_use, vol7, ret_lag1, mom7]], columns=FEATURES[ALGO_GB])
    ...
    if len(results) == 0:
        ...
        if len(results) == 1:
            ...
            res_df = pd.DataFrame(list(results.items()), columns=["Algorithm", "Predicted Price"]).set_index("Algorithm")
    ...
    if len(forecasts) == 0:
        ...
        st.caption("Each line is a recursive day-by-day forecast seeded only from that algorithm's inputs.")
